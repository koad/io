...
- [x] `harness/pi/command.sh` — validated. `PI_CODING_AGENT_DIR` confirmed via source; binary `pi`, flags `--no-context-files`, `--model provider/id`, `-p` for one-shot all confirmed.
