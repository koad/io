...
# ---------------------------------------------------------------------------
#
# Signal chain:  Claude Code renders UI → pipes JSON payload on stdin
#                → this script does two things:
#
#   1. DISPLAY — renders a 3-row ANSI statusline (starship rows + sensor row)
#   2. SENSOR  — writes two files to disk (rooted entities only):
#
#      dispatch-control.json       — singleton, overwritten every turn
#        → read by dispatch hooks for pre-flight cost/token baseline
#        → contains raw Claude Code payload (cost, context, model, rate_limits)
#
#      sessions/<session_id>.json  — one file per session, carries pid ($PPID)
#        → read by daemon session-scanner to populate HarnessSessions
#        → each concurrent terminal writes its own file with its own pid
#        → the scanner creates one entity:host:pid record per session
#
# Does NOT emit to the daemon directly. Writes to disk only.
# The daemon's session-scanner.js watches the sessions/ directory.
#
# Wiring (in ~/.claude/settings.json):
#   { "statusLine": { "type": "command", "command": "<path>/statusline.sh" } }
#
# Payload fields (Claude Code 2.1.x):
...
# Path is XDG-compliant: ~/.<entity>/.local/state/harness/
...
if [ -n "$PAYLOAD" ] && [ -n "$_entity_dir_for_outfit" ] \
  ...
  if mkdir -p "$_sensor_dir" 2>/dev/null; then
    # dispatch-control.json — pre-flight snapshot read by dispatch hooks for
    # baseline cost/token capture. Singular, overwritten every turn.
    _sensor_tmp="$_sensor_dir/.dispatch-control.json.tmp.$$"
    if printf '%s' "$PAYLOAD" > "$_sensor_tmp" 2>/dev/null; then
      mv -f "$_sensor_tmp" "$_sensor_dir/dispatch-control.json" 2>/dev/null \
    ...
    # Inject the harness PID so the scanner can create canonical pid-keyed
    # records without relying on the singleton harness.pid file.
    ...
    if [ -n "$_session_id" ]; then
      ...
      if mkdir -p "$_sessions_dir" 2>/dev/null; then
        _session_payload=$(printf '%s' "$PAYLOAD" | jq -c \
          --arg pid "${KOAD_IO_HARNESS_PID:-0}" \
          --arg spirit "${KOAD_IO_SPIRIT:-${USER:-unknown}}" \
          '. + {pid: ($pid | tonumber), spirit: $spirit}' 2>/dev/null)
        [ -z "$_session_payload" ] && _session_payload="$PAYLOAD"
        _session_tmp="$_sessions_dir/.${_session_id}.json.tmp.$$"
        if printf '%s' "$_session_payload" > "$_session_tmp" 2>/dev/null; then
