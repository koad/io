...
migrate_entity_dispatch() {
  ...
  if [ ! -f "$passenger_file" ]; then
    ...
    has_dispatch=$(jq -r 'if .dispatch.color and .dispatch.model then "yes" else "no" end' "$passenger_file" 2>/dev/null || echo "no")
  ...
  if [ "$need_passenger" -eq 1 ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
      ...
      if [ -f "$passenger_file" ]; then
        ...
        jq -r '"        dispatch: " + (.dispatch // "(missing)" | tojson)' "$passenger_file"
      else
...
regen_entity() {
  ...
  has_dispatch=$(jq -r 'if .dispatch.color and .dispatch.model then "ok:" + .dispatch.color + ":" + .dispatch.model else "missing" end' "$passenger_file" 2>/dev/null || echo "missing")
