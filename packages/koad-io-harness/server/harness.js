/**
 * koad:io-harness — Harness Bridge Protocol (VESTA-SPEC-191)
 *
 * Provides bidirectional harness communication for running opencode sessions:
 *
 *   sidecar.py  — outbound telemetry (opencode → daemon). EXISTING.
 *   bridge.js   — inbound command delivery (daemon → opencode TUI). NEW.
 *
 * This package owns:
 *   - HarnessCommands in-memory collection (ephemeral, no disk persistence)
 ...
 *   - 60-second pruning interval (removes executed/failed docs older than 5 min)
 *   - 30-second SSE keepalive heartbeat (harness_ping events)
 */
...
// ---------------------------------------------------------------------------
// HarnessCommands — in-memory only, no disk persistence
// ---------------------------------------------------------------------------

const HarnessCommands = new Mongo.Collection('HarnessCommands', { connection: null });

// Soft cap per (entity, sessionId)
const QUEUE_CAP = 50;

// Prune completed docs older than this
...
function makeCommandId() {
  // "cmd_" + 16 random hex chars
  return 'cmd_' + Random.hexString(16);
}
...
async function deliverCommand(doc) {
  ...
  const sent = writeSseFrame(streams[doc.entity][doc.sessionId].res, 'harness_command', frame);
  if (sent) {
    streams[doc.entity][doc.sessionId].lastFrameAt = new Date();
    await HarnessCommands.updateAsync(doc._id, {
...
// Prune old completed commands from in-memory collection
Meteor.startup(() => {
  Meteor.setInterval(async () => {
    const cutoff = new Date(Date.now() - PRUNE_AGE_MS);
    await HarnessCommands.removeAsync({
...
app.use('/harness/ping', (req, res, next) => {
  ...
  // Only handle exact /harness/ping
  if (fullPath !== '/harness/ping' && fullPath !== '/harness/ping/') return next();
  if (req.method !== 'GET') return next();

  sendJson(res, 200, {
    status: 'ok',
    activeBridges: Object.values(_bridgeStreams).reduce(
      (acc, sessions) => acc + Object.keys(sessions).length, 0
    ),
    queuedCommands: HarnessCommands.find({ status: 'queued' }).count(),
...
app.use('/harness/stream', async (req, res, next) => {
  ...
  if (_bridgeStreams[entity] && _bridgeStreams[entity][sessionId]) {
    // Send error on the NEW connection, keep the existing one
  ...
  const queued = await HarnessCommands.find(
    { entity, sessionId, status: 'queued' },
    { sort: { enqueuedAt: 1 } }
  ).fetchAsync();
  for (const doc of queued) {
    await deliverCommand(doc);
  }
...
app.use('/harness/commands', async (req, res, next) => {
  ...
  // Also handles PUT /harness/commands/:id/status — dispatch below
  ...
  if (putParams && req.method === 'PUT') {
    ...
    const doc = await HarnessCommands.findOneAsync(id);
    if (!doc) {
    ...
    if (status === 'executed') {
      ...
      if (body.reason) update.failReason = body.reason;
    }

    await HarnessCommands.updateAsync(id, { $set: update });
    res.writeHead(204);
    return res.end();
  }
  ...
  if (postParams && req.method === 'POST') {
    ...
    const queuedCount = await HarnessCommands.find(
      { entity, sessionId, status: 'queued' }
    ).countAsync();
    ...
    await HarnessCommands.insertAsync(doc);
    ...
    const httpStatus = bridgeOpen ? 202 : 404;
    // Still 202 semantics even on 404: command is queued. But spec says 404
    // to signal no bridge registered. We use 202 always (command is queued).
    // Spec §4.3.2: "HTTP 404 if no bridge is registered for the session
    // (command is still queued — bridge will receive it on reconnect)."
...
app.use('/harness/bridge', async (req, res, next) => {
  ...
  if (req.method === 'POST') {
    ...
    // Update HarnessSessions if available (VESTA-SPEC-142 extension, SPEC-191 §4.4)
    ...
    if (HarnessSessions) {
      ...
      if (existing) {
        ...
        // No session record yet — not fatal, bridge continues
        return sendJson(res, 404, { status: 'not_found', message: 'No HarnessSessions record for this sessionId. Bridge continues.' });
      }
    } else {
      // HarnessSessions not yet available (session-history package not loaded)
