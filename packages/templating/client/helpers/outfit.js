...
// ---------------------------------------------------------------------------
// EntityColor — warm hsl accent color for an entity handle.
// Reads hue from Passengers collection (same logic as KoadOverview._entityColor).
// Returns a CSS hsl() string: e.g. "hsl(270, 60%, 65%)"
//
// Usage: {{EntityColor handle}}   or   style="color: {{EntityColor entity}}"
// Falls back to hsl(200, 60%, 65%) when passenger record is absent.
// ---------------------------------------------------------------------------
Template.registerHelper('EntityColor', function(handle) {
  var Passengers = globalThis.Passengers || null;
  var passenger = Passengers ? Passengers.findOne({ handle: handle }) : null;
  var hue = passenger && passenger.outfit ? passenger.outfit.hue : 200;
  return 'hsl(' + hue + ', 60%, 65%)';
});

// ---------------------------------------------------------------------------
// AccentColor — full-outfit hsl accent color for an entity handle.
// Reads hue + saturation + brightness from Passengers collection outfit.
// Returns a CSS hsl() string with brightness capped at 60%.
//
// Usage: {{AccentColor handle}}
// Falls back to hsl(0, 0%, 50%) when passenger record is absent.
// ---------------------------------------------------------------------------
Template.registerHelper('AccentColor', function(handle) {
  var Passengers = globalThis.Passengers || null;
  var passenger = Passengers ? Passengers.findOne({ handle: handle }) : null;
  var outfit = passenger && passenger.outfit;
  var hue = outfit ? outfit.hue : 0;
  var sat = outfit ? outfit.saturation : 0;
  // brightness is not stored in passenger.outfit — accent clips at 60% (Math.min(69+20, 60)).
