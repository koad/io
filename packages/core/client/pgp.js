 ...
 *
 * @param {string} armoredPubKey - Armored PGP public key block
 * @returns {Promise<{fingerprint: string, error?: string}>}
 */
async function fingerprint(armoredPubKey) {
  if (typeof armoredPubKey !== 'string') {
    throw new TypeError('[koad.deps.pgp.fingerprint] armoredPubKey must be a string');
  }

  const kbpgp = await _loadKbpgp();

  return new Promise(function(resolve) {
    kbpgp.KeyManager.import_from_armored_pgp({ armored: armoredPubKey }, function(err, km) {
      if (err) {
        return resolve({ fingerprint: '', error: err.message });
      }
      try {
        const fpBuf = km.get_pgp_fingerprint();
        const fp = fpBuf ? fpBuf.toString('hex').toUpperCase() : '';
        resolve({ fingerprint: fp });
      } catch (e) {
        resolve({ fingerprint: '', error: e.message });
      }
    });
  });
}
...
koad.deps = koad.deps || {};
koad.deps.pgp = { clearsign, verify, fingerprint };

export { clearsign, verify, fingerprint };
