...
function project(collection, entries, config) {
  ...
  const keyResolution = config.key_resolution || 'last-entry';

  if (mode === 'current-per-key') {
    // Build a map: keyValue → winning entry with that key.
    // Default is append-order last-entry. key_resolution: last-timestamp chooses
    // the record with the greatest timestamp field for the same key.
    const latest = {};
    for (const entry of entries) {
      const kv = resolveKey(entry, key);
      if (kv === null) continue;

      if (keyResolution === 'last-timestamp' && latest[kv]) {
        const oldTime = Date.parse(latest[kv].timestamp || '');
        const newTime = Date.parse(entry.timestamp || '');
...
// ---------------------------------------------------------------------------
// Hot reload — diff configs by name AND config hash.
// - Removed name → stop
// - New name → start
// - Same name, changed config hash → stop old, start with new config
// - Same name, same config hash → no-op (watcher still active, no churn)
// ---------------------------------------------------------------------------
...
function reload(newConfigs) {
  ...
  for (const cfg of newConfigs) {
    if (!oldNames.has(cfg.name)) {
      ...
      // Check config hash to detect changed source/format/mode/key
      const running = _running[cfg.name];
      const hashFn = globalThis.IndexerRegistry && globalThis.IndexerRegistry.configHash;
      const oldConfig = running && running.config;
      ...
      if (newHash && oldHash && newHash !== oldHash) {
        console.log(`[jsonl-projector] reload: ${cfg.name} config changed (${oldHash} → ${newHash}) — restarting`);
        stop(cfg.name);
        start(cfg);
      } else {
        console.log(`[jsonl-projector] reload: ${cfg.name} unchanged — no-op`);
      }
    }
  }
}
