...
export function registerKingdomQueryTools(pi: ExtensionAPI, ddp: DDPClient | null): void {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      const result = await missionQuery(ddp, {
        id: params.id,
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      const result = await sessionQuery(ddp, {
        id: params.id,
        entity: params.entity,
        active_only: params.active_only,
        limit: params.limit || 50,
  ...
  pi.registerTool({
    ...
    renderResult(result: any, { expanded }: any, theme: any) {
      ...
      if (expanded && details.results?.length) {
        for (const e of details.results.slice(0, 10)) {
          ...
          lines.push(`  ${theme.fg("accent", e.entity)} ${theme.fg("dim", `[${typeLabel}]`)} ${clip(e.body || "", 50)}`);
        }
      }
      return new Text(lines.join("\n"), 0, 0);
    },
    ...
    async execute(_toolCallId, params) {
      const result = await emissionQuery(ddp, {
        id: params.id,
        entity: params.entity,
        type: params.type,
        status: params.status,
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      const result = await bondQuery(ddp, {
        entity: params.entity,
        from: params.from,
        to: params.to,
        type: params.type,
  ...
  pi.registerTool({
    ...
    renderResult(result: any, { expanded }: any, theme: any) {
      ...
      if (expanded && details.results?.length) {
        for (const q of details.results.slice(0, 10)) {
          ...
          lines.push(`  ${icon} ${theme.fg("accent", q.from)}→${theme.fg("accent", q.to)} ${theme.fg("dim", clip(q.question, 50))}`);
        }
      }
      return new Text(lines.join("\n"), 0, 0);
    },
    ...
    async execute(_toolCallId, params) {
      const result = await questionQuery(ddp, {
        from: params.from,
        to: params.to,
        status: params.status,
        id: params.id,
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      const result = await entityQuery(ddp, {
