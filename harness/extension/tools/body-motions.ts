...
export function registerBodyTools(pi: ExtensionAPI): void {
    ...
    async execute(_toolCallId, params) {
      ...
      if (att.high_priority_obligations?.length) {
        ...
        for (const o of att.high_priority_obligations.slice(0, 5)) {
          lines.push(`  - ${o.title || o.id} (id: ${o.id})`);
        }
      }
      return {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const lines: string[] = [];
      lines.push(`${parsed.entity || "?"} inbox: ${total} items — ⚡${buckets.actionable?.count ?? 0} actionable, 🧠${buckets.decision_bearing?.count ?? 0} decisions, 📦${buckets.stale?.count ?? 0} stale`);

      // Surface actionable items inline
      const actionableItems = buckets.actionable?.items ?? [];
      ...
      const decisionItems = buckets.decision_bearing?.items ?? [];
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const buckets = parsed.buckets ?? {};
      const lines: string[] = [];
      lines.push(`${parsed.entity || "?"} obligations: ${total} total — 🔴${buckets.overdue?.count ?? 0} overdue, 🟡${buckets.today?.count ?? 0} today, 📦${buckets.stale?.count ?? 0} stale`);

      // Surface overdue items inline — these are what the orchestrator needs to act on
      const overdueItems = buckets.overdue?.items ?? [];
      ...
      // Surface today items too
      const todayItems = buckets.today?.items ?? [];
      ...
      // Surface stale items — they're forgotten, need triage
      const staleItems = buckets.stale?.items ?? [];
  ...
  pi.registerTool({
    ...
    label: "Flight Log",
    description: [
      "Read a dispatched flight's run record — completion summary, stderr tail,",
      "files touched, and session transcript tail.",
      "Backed by ~/.forge/commands/flight/log/command.sh",
    ].join("\n"),
    promptSnippet: "Show what a dispatched flight actually did",
    promptGuidelines: [
      "Use flight_log after a flight lands to see what happened.",
      "Pass the flight_id from dispatch or mission_query output.",
      "Returns completion summary, close reason, files touched, stderr tail.",
      "Use --lines=N to control transcript tail length (default 40).",
    ],
    parameters: Type.Object({
      flight_id: Type.String({
        description: "Flight ID (from dispatch or mission_query).",
      }),
      lines: Type.Optional(Type.Number({
        description: "Max transcript lines to return (default 40).",
      })),
    }),
    ...
    renderResult(result: any, { expanded }: any, theme: any) {
      ...
      const ok = details.exitCode === 0;
      const lines = [
        theme.fg(ok ? "success" : "error", ok ? `✈️ ${details.entity || "?"} [${details.status || "?"}]` : `✗ flight not found`),
      ];
      if (ok) {
        lines.push(`  ${theme.fg("dim", `${details.elapsed || 0}s · ${details.brief || ""}`)}`);
        if (details.completion_summary) lines.push(`  ${theme.fg("dim", clip(details.completion_summary, 120))}`);
      }
      return new Text(lines.join("\n"), 0, 0);
    },

    async execute(_toolCallId, params) {
      const args = ["flight", "log", params.flight_id, "--json"];
      ...
      if (result.exitCode !== 0 || !parsed) {
        return {
          content: [{ type: "text", text: result.stdout || result.stderr || "flight_log failed" }],
          details: { exitCode: result.exitCode, stderr: result.stderr.slice(0, 500) },
        };
      }
      const lines: string[] = [];
      lines.push(`✈️ ${parsed.entity || "?"} [${parsed.status || "?"}] — ${parsed.elapsed || 0}s`);
      ...
      if (parsed.stderr_tail) {
        lines.push("");
        lines.push(`stderr: ${parsed.stderr_tail.slice(0, 500)}`);
      }
      if (parsed.output_files?.length) {
        lines.push("");
        lines.push(`files (${parsed.output_files.length}):`);
        for (const f of parsed.output_files.slice(0, 15)) {
          lines.push(`  ${f}`);
        }
      }
      return {
