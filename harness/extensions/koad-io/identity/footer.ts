...
import { compactModel, fmtTok, fmtWindow, sanitizeStatusText } from "../utils/format";
...
import { getNowPlaying } from "../music";
...
export function renderFooter(
  ...
  if (g.isRepo) {
    // Remote URL
    if (g.remote) r2 += ` ${dim(g.remote)}`;

    // Branch
    if (g.branch) r2 += ` ${magenta("🌱")} ${white(g.branch)}`;

    // Git status counters
    if (g.staged    > 0) r2 += ` ${brightGreen("📦" + g.staged)}`;
    if (g.deleted   > 0) r2 += ` ${brightRed("🗑️" + g.deleted)}`;
    if (g.modified  > 0) r2 += ` ${yellow("●" + g.modified)}`;
    if (g.untracked > 0) r2 += ` ${brightCyan("🌱" + g.untracked)}`;
    if (g.conflicted> 0) r2 += ` ${brightRed("⚠" + g.conflicted)}`;
    if (g.ahead     > 0) r2 += ` ${dim("⇡" + g.ahead)}`;
    if (g.behind    > 0) r2 += ` ${dim("⇣" + g.behind)}`;
  }
  ...
  let r2right = "";
  if (tel.tokensIn)   r2right += `${dim("↑")}${white(fmtTok(tel.tokensIn))} `;
  if (tel.tokensOut)  r2right += `${dim("↓")}${white(fmtTok(tel.tokensOut))} `;
  if (tel.cacheRead)  r2right += `${dim("R")}${white(fmtTok(tel.cacheRead))} `;
  if (tel.cacheWrite) r2right += `${dim("W")}${white(fmtTok(tel.cacheWrite))} `;
  if (tel.totalCost > 0) r2right += `${green(`$${tel.totalCost.toFixed(3)}`)} `;
  if (tel.contextPct > 0) {
    const pctStr = tel.contextWindow > 0
      ? `${tel.contextPct}%/${fmtWindow(tel.contextWindow)}`
      : `${tel.contextPct}%`;
    r2right += `${dim("c")}${ctxColor(tel.contextPct, pctStr)} `;
  }
