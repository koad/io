...
export function createTelemetrySession(pi: ExtensionAPI, ddp: DDPClient): TelemetrySession {
  ...
  const daemonHttpUrl = process.env.KOAD_IO_DAEMON_URL ?? "http://10.10.10.10:28282";
  const controlHttpUrl = process.env.KOAD_IO_CONTROL_URL ?? "http://10.10.10.10:28283";
  ...
  function daemonUpdate(payload: Record<string, unknown>): void {
    if (!emitEnabled || !emissionId) return;
    fetch(`${daemonHttpUrl}/emit/update`, {
      method: "POST",
  ...
  pi.on("agent_start", (_event: any, ctx: any) => {
    ...
    daemonUpdate({ status_line: `thinking | ${entity} | ${compactModel(id.currentProvider, id.currentModel)}` });
    refresh();
  });
  ...
  pi.on("agent_end", (_event: any, ctx: any) => {
    ...
    storeCtx(ctx);
    try { tel.autoCompact = ctx.sessionManager?.getSession?.()?.autoCompactionEnabled ?? tel.autoCompact; } catch (_) {}
    daemonUpdate({ status_line: `idle | t${tel.turnCount} $${tel.totalCost.toFixed(4)}` });
    flushSession();
    refresh();
  });
  ...
  pi.on("tool_execution_start", (event: any, ctx: any) => {
    ...
    daemonUpdate({ note: `→ ${note}`, status_line: `${entity}│⚙${note.slice(0, 40)}` });
    storeCtx(ctx);
    refresh();
  });
  ...
  pi.on("tool_execution_end", (event: any, ctx: any) => {
    if (event.isError) {
      ...
      daemonUpdate({ note: `← ${event.toolName} ERROR` });
    }
