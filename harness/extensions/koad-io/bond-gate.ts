...
//   6. No matching bonds → env-var fallback → default (koad:io tools only)
//
// Tool classification:
//   - koad:io tools     → always allowed (daemon-mediated)
...
function mergeBondScope(entity: string, bonds: ParsedBond[], errors: string[], interactive: boolean): BondScope {
  ...
  if (interactive) {
    if (intOverride.bash !== undefined) tools.bash = intOverride.bash;
    if (intOverride.exec) {
...
function resolveGate(entity: string, interactive: boolean): BondScope {
  ...
  return {
    ...
    errors: [],
    mode: "default",
    label: "mode=default — no bonds, koad:io tools only",
    bondCount: 0,
  };
}
...
export function registerBondGate(pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    // ── koad:io ecosystem tools — always allowed ─────────────
    if (KOADIO_TOOLS.has(toolName)) return undefined;
    ...
    // ── Read tools — cwd + children readable if cwd in bond scope ─
    if (FILE_READ_TOOLS.has(toolName)) {
      // cwd is readable only when it's within bond read scope
      if (absolutePath && scope.file.read.length > 0 && isUnder(ctx.cwd, scope.file.read) && isUnder(absolutePath, [ctx.cwd])) {
